...
const int	GENERAL_BUFFER_LEN		= 4096;
...
#define		DISPLAY_PAGE_NAME	"exoplanets.swt"
#define		GRAPH_PROG_FILEPATH	"/usr/bin/java"
#define		EXOPLANET_DATA_URL	"\"https://exoplanetarchive.ipac.caltech.edu/cgi-bin/nstedAPI/nph-nstedAPI?table=exoplanets&select=pl_name,pl_orbper,pl_orbeccen,pl_bmassj,pl_radj,st_dist,st_teff,st_mass,st_rad&format=json\""
...
class		UserContent
{
  ...
  //  PURPOSE:  To hold the process id of the graph making process.
  pid_t				graphPid_;

  //  PURPOSE:  To hold the file descriptor for talking to the graph making
  //	process.
  int				toGraphFd_;

  //  PURPOSE:  To hold the file descriptor for listening to the graph making
  //	process.
  int				fromGraphFd_;
...
public :
  ...
  //  PURPOSE:  To send the request encoded in the JSON string 'textCPtr' to
  //	the graphing process, get the response from that process, and return
  //	a heap-allocated JSON interpretation of that response.
  JSONValue*
	requestGraph	(const char*	textCPtr
  			)
			{
			  fprintf(stderr,"httpServer -> graphServer: %s\n\n",
			  	  textCPtr
				 );
			  write(toGraphFd_,textCPtr,strlen(textCPtr));

			  return(JSONValue::factory(fromGraphFd_,false));
			}
...
UserContent::UserContent	(const char*	filepathCPtr,
				...
				graphPid_(-1),
				toGraphFd_(ERROR_FILE_DESCRIPTOR),
				fromGraphFd_(ERROR_FILE_DESCRIPTOR)
{
  //  I.  Application validity check:
  ...
  //  II.B.  Launch graph-making process:
  //  II.B.1.  Create socket-pair to communicate with process:
  int	fdPair[2];

  socketpair(AF_UNIX,SOCK_STREAM,0,fdPair);

  //  II.B.2.  Launch graph-making process:
  graphPid_	= fork();
  ...
  {
    close(fdPair[0]);
    dup2(fdPair[1], STDIN_FILENO);
    dup2(fdPair[1],STDOUT_FILENO);
    //fprintf(stderr,"Executing: %s %s -Dhttps.proxyHost=proxy.is.depaul.edu -Dhttps.proxyPort=312 -cp .:/usr/share/java/jsoup-1.12.1.jar:/usr/share/java/gson-2.8.6.jar: graphServer %s %s",GRAPH_PROG_FILEPATH,GRAPH_PROG_FILEPATH, EXOPLANET_DATA_URL,NULL);
    execl(GRAPH_PROG_FILEPATH,GRAPH_PROG_FILEPATH,"-Dhttps.proxyHost=proxy.is.depaul.edu", "-Dhttps.proxyPort=3128","-cp",".:/usr/share/java/jsoup-1.12.1.jar:/usr/share/java/gson-2.8.6.jar:", "graphServer",EXOPLANET_DATA_URL,NULL);
    //fprintf(stderr,"Executed: %s %s -Dhttps.proxyHost=proxy.is.depaul.edu -Dhttps.proxyPort=312 -cp .:/usr/share/java/jsoup-1.12.1.jar:/usr/share/java/gson-2.8.6.jar: graphServer %s %s",GRAPH_PROG_FILEPATH,GRAPH_PROG_FILEPATH, EXOPLANET_DATA_URL,NULL);
  ...
  close(fdPair[1]);
  toGraphFd_	= fromGraphFd_	= fdPair[0];
...
UserContent::~UserContent	()
{
  ...
  //  II.  Release resources:
  delete(requestGraph("{\"command\":\"quit\"}"));
  wait(NULL);
  close(toGraphFd_);
				...
				)
{
  ...
  if  ( strncmp(filePath,DISPLAY_PAGE_NAME,strlen(DISPLAY_PAGE_NAME)) == 0)
  {
    std::map<std::string,std::string>::const_iterator	found;
    std::string						xAxisStr = "";
    std::string						yAxisStr = "";

    found	= request.getQuery().find("xAxis");

    if  (found != request.getQuery().end())
    {
      xAxisStr	= found->second;
    }


    found	= request.getQuery().find("yAxis");

    if  (found != request.getQuery().end())
    {
      yAxisStr	= found->second;
    }


    if  ( !xAxisStr.empty() && !yAxisStr.empty() )
    {
      char		text[GENERAL_BUFFER_LEN];
      JSONValue*	responsePtr;
      const JSONValue*	filenamePtr;
      const JSONValue*	widthPtr;
      const JSONValue*	heightPtr;

      snprintf
	(text,
	 GENERAL_BUFFER_LEN,
	 "{\"command\":\"plot\",\"xAxis\":\"%s\",\"yAxis\":\"%s\"}",
	 xAxisStr.c_str(),yAxisStr.c_str()
	);
      responsePtr	= content.requestGraph(text);
      filenamePtr	= responsePtr->getElement("imageFilename");
      widthPtr		= responsePtr->getElement("width");
      heightPtr		= responsePtr->getElement("height");

      request.getQuery()["imageFilename"]	= filenamePtr->getString(false);
      request.getQuery()["width"]		= widthPtr->getString(false);
      request.getQuery()["height"]		= heightPtr->getString(false);
      delete(responsePtr);
    }
    else
    {
      system("cp noData.png barChart.png");
    }
  }
  else
  if  ( strcmp(filePath,LOGIN_PAGE_NAME) == 0)
  {
    const char*	cPtr;
    std::map<std::string,std::string>::const_iterator
				...
				)
{
  ...
  const char*	exoplanetUrlCPtr	= EXOPLANET_DATA_URL;
  ...
  //  II.B.1.  Initialize 'toUomJsonPath' and 'toTempValueJsonPath':
  ...
  UserContent	content(contentFilepathCPtr,exoplanetUrlCPtr);
  SessionStore	sessionStore;
